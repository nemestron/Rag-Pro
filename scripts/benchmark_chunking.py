...
def run_benchmark():
    ...
    print(f"Document loaded successfully. Total length: {len(docs[0].page_content)} characters.")
    ...
    for config in configs:
        print("\n" + "-"*50)
        print(f"TESTING CONFIG: Chunk Size = {config['size']}, Overlap = {config['overlap']}")
        print("-" * 50)
        
        # Measure Chunking
        start_chunk = time.time()
        chunks = chunk_documents(docs, chunk_size=config['size'], chunk_overlap=config['overlap'])
        ...
        print("\nRetrieval Sample (Top Result Length):", len(retrieved_docs[0].page_content), "characters")
