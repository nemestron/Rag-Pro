...
def main():
    ...
    st.set_page_config(
        page_title="RAG Pro", layout="wide", initial_sidebar_state="expanded"
    )
    ...
    with st.sidebar:
        ...
        if source_type == "URL":
            url_count = st.number_input(
                "Number of URLs to process", min_value=1, max_value=10, value=1
            )
            ...
            raw_text = st.text_area(
                "Paste raw text here", height=250, placeholder="Enter text content..."
            )
            ...
            if source_type == "URL" and not urls:
                ...
                with st.spinner("Processing documents..."):
                    try:
                        ...
                        st.success(
                            f"Successfully processed {len(docs)} documents into {len(chunks)} searchable chunks!"
                        )
    ...
    if not st.session_state.is_processed:
        st.info(
            "Please select a data source and click 'Process Documents' in the sidebar to begin."
        )
                ...
                if message["role"] == "assistant" and "context" in message:
                    with st.expander("View Retrieved Context"):
                        for i, chunk in enumerate(message["context"]):
                            st.markdown(
                                f"**Chunk {i+1} (Source: {chunk.metadata.get('source_name', 'Unknown')})**"
                            )
                    ...
                    try:
                        ...
                        with st.expander("View Retrieved Context"):
                            for i, doc in enumerate(retrieved_docs):
                                st.markdown(
                                    f"**Chunk {i+1} (Source: {doc.metadata.get('source_name', 'Unknown')})**"
                                )
