...
if __name__ == "__main__":
    ...
    try:
        test_retriever = RunnableLambda(
            lambda query: [
                Document(
                    page_content="Retrieval-Augmented Generation bridges the gap between static LLMs and external knowledge bases."
                )
            ]
        )
        ...
        # Test 1: Valid Query
        print("\n[Test 1: Valid Query]")
        valid_answer = ask_question(
            "What does RAG bridge the gap between?", test_retriever
        )
