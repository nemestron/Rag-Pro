...
def chunk_documents(
    ...
    try:
        logger.info(f"Initializing RecursiveCharacterTextSplitter (size={chunk_size}, overlap={chunk_overlap}).")
        ...
        if not chunks:
            logger.warning("Chunking completed but returned no chunks. Check input document content.")
            return []
            
        logger.info(f"Successfully split {len(documents)} source documents into {len(chunks)} chunks.")
        return chunks
